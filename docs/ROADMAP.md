...
5. **Close out M0.** #11 gap verification (passive, over the following days), #59 health-check query indexes (deferred from #5/#6: the checks read the whole oversized raw table today; index builds are cheap once the prune shrinks it), #2 postmortem, #9 gitignore, #45 README badge. (#10, the merge to main, was pulled forward and done on 2026-07-12 when the umbrella-branch model was retired.)
