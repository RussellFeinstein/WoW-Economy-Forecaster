...
7. Deploy the trigger Worker (issue #83), so capture no longer depends on GitHub's schedule. Create a fine-grained PAT scoped to this repo with the Actions permission set to read and write, and **set its expiration to none**: GitHub's creation UI defaults that dropdown to 30 days, which is how the first token expired unannounced on 2026-08-22 (the reasoning for choosing no expiration, and the rotation procedure, are in [../cloud-trigger/README.md](../cloud-trigger/README.md)). Then from [../cloud-trigger/](../cloud-trigger/): `wrangler secret put GH_PAT` (paste the PAT), `wrangler deploy`. The non-secret config is already in `wrangler.toml`. Confirm with `wrangler tail wow-forecaster-cloud-trigger` and the Actions tab that dispatch runs appear at :16 and :46.
