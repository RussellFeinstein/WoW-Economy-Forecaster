...
- **#13 says "Migration 0009"**: 0009 was consumed by the health-check indexes (v2.7.4, issue #59) and 0010/0011 by the normalized obs_id FK index and the raw item_id index drop (v2.14.20 and v2.14.22, issues #149 and #153). The ledger migration is **0012**. Comment on #13 at PR time.
...
- **Migrations**: a single module wow_forecaster/db/migrations.py with a registry dict; the highest is 0011. New tables go in BOTH schema.py (fresh DBs) and migrations.py (existing DBs). The ledger migration is `0012_forecast_realizations`.
...
|----|-------|--------|------|-----------|-------|
| 1 | #13 ledger | feat/13-forecast-realizations-ledger | minor | none | Migration 0012. Keystone: unblocks #14, #101, #15, #19 evidence, and M2. Detailed design below. |
...
- Comment on #13: the migration number is 0012 (0009 was taken by #59, 0010 by #149, 0011 by #153); the scoreable-today count is ~155K of 318K matured, and the rest are realized_source='missing'.
...
- Per-PR Documentation Sync: CHANGELOG under [Unreleased]; repo CLAUDE.md (table count 23 to 24 and "migrations end at 0012" in PR 1; CLI command count; layer summaries; test counts); README for new user-facing commands; milestone work-order list updates when issues close.
...
### Schema (migration `0012_forecast_realizations` plus identical DDL in schema.py, the model_health_snapshots dual-site pattern)
...
- `tests/test_db/test_migration_0012.py` (mirror test_migration_0009.py): registry entry; apply_schema creates the table and indexes; upgrade path via run_migrations on a pre-0012 DB; ALL_TABLE_NAMES parity; UNIQUE enforcement; CHECK rejections; EXPLAIN QUERY PLAN pins for the coverage and model-slicing queries (assert seek terms where the 07-22 lesson applies).
...
1. Branch off latest main. 2. Write failing tests. 3. schema.py DDL plus ALL_TABLE_NAMES plus docstring count 23 to 24. 4. migrations.py `0012_forecast_realizations`. 5. realization.py module. 6. cli.py command. 7. Full suite green (`pytest --tb=no`, no extra -q; lint `ruff check wow_forecaster/ tests/`). 8. Docs: CHANGELOG [Unreleased]/Added; README CLI section; repo CLAUDE.md (24 tables, migrations end at 0012, command count, M1 layer note, test count). 9. Live acceptance on rex-desktop (below). 10. Stamp commit plus PR `(vX.Y.Z)` plus comment the audit corrections on #13.
