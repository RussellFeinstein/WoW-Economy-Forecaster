...
- Item-forecast reads (v2.14.28, issue #107): `_fetch_items_with_history`, `_fetch_item_prices` and `_fetch_archetype_prices` read `daily_rollup_item` / `daily_rollup_archetype`, NOT market_observations_normalized, through module-level SQL constants (`_ITEMS_WITH_HISTORY_SQL` and siblings, exposed by `_*_sql()` builders so tests pin the production strings). Exact by the #123 argument, with the other column pair: these queries always filtered `price_gold > 0`, so `price_obs_count_pos > 0` is the per-day form of that filter and `qty_weighted_price_sum_pos / qty_weight_sum_pos` is the old weighted mean (the baseline in normalize.py is zero-inclusive and uses `price_sum / obs_count`; picking the wrong pair changes the statistic silently). The history count is lifetime, which is v1.12.0's definition; between the retention prune (#149) and #107 it was a rolling 30-day count because that was all the normalized table held (10,303 eligible items against 9,146 on the production DB). `_generate_item_forecasts` runs on a read-only connection BEFORE the write transaction opens, so the transaction holds inserts only; it warns when candidates exist and the 7-day rollup window prices none of them (a stalled rollup step), naming `backfill-rollups`. `_execute(now=...)` also anchors the price window when given. The freshness gate stays on the normalized table on purpose (single-MAX seek, own connection)
