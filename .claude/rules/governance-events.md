...
- **Migration 0011 dropped `idx_obs_raw_item_time`** (issue #153). Nothing queries `market_observations_raw` by item_id (every read filters on `observed_at`, `is_processed` or `realm_slug`), so it cost maintenance on every insert and every prune delete for no reader, and it was the only structure implicated when the table corrupted on 2026-08-22, blocking the prune. The DDL is gone from schema.py too, and that pairing is load-bearing: `init-db` runs `apply_schema()` before `run_migrations()` and the constant uses `IF NOT EXISTS`, so a line there silently outranks the migration. Do not reintroduce it without a query that needs it
- **Migration 0012 dropped `idx_obs_raw_realm_ingested`** (issue #155). Two of its entries were missing on the production database; when their hour slice (2026-07-31T06) entered the retention window on 2026-08-31 the raw DELETE for that slice raised `database disk image is malformed` on every hourly run, and because the slice loop stops at the first failure nothing behind it was pruned for twenty days (about 105M raw rows past the ToS window, file 98 GB -> 150 GB). Its one reader, the health check's freshness probe, moved to the normalized table, so the drop is the same smallest-write repair as 0011 and the schema.py DDL left in the same change. The discriminator verdict was real damage by a stronger test than the runbook's two passes: the same DELETE failing identically 364 times. The backlog was cleared on 2026-09-20 by 55 `prune-snapshots` calls (about 1.6M rows and 80 s each, 13:20 to 14:44), with the evidence on #155; the file stays at 150 GB with 43% of its pages on the freelist. Filed from the incident: #160 (skip and report a failing slice instead of stalling every later one), #161 (the daily forecast gate should not block on retention), #162 (the hourly run overruns its slot on an oversized normalized table)
