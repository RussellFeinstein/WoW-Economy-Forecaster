...
- Three learning-bank questions that cited the old Work order section (m13-q17, m20-q11, m20-q17) are re-anchored onto the sentences in the rewritten section that carry the same rules, and each answer gains a paragraph on where the order lives now. The drift guard caught all three in the suite, which is the mechanism working as designed; no question was removed

### Notes
- Auditing the old lists before retiring them surfaced that [#72](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/72) is live: today's hourly logs `Event shock | active=13` and the forecast stage applies `drift CI multiplier=1.25`, so the open-ended-event floor is on every forecast being issued. It carries the `live` flag at the head of M5 on the tracker, in the milestone it was filed to
...
- Three issues closed on evidence that had been sitting past their gates: [#123](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/123) on production timings that had been in `hourly.log` since 07-31 (normalize stage 49.5 minutes on 07-30, 27 seconds on 09-20 on a table five times larger); [#143](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/143) once the 09-01 Dependabot PR arrived carrying only taxonomy labels, after which `dependencies`, `python` and `github_actions` were removed from the three merged PRs that held them and deleted, leaving 19 labels (the docs said 21 because the count included two of the defaults); and [#42](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/42) on a read-only bucket listing showing nothing older than 30 days. [#11](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/11) came off the clock with one check left. All four had passed their dates unread, which is [#146](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/146)'s case four times over

### Notes
- The #155 remainder did not drain over the following days as the 2.14.25 notes expected: it was cleared the same afternoon by 55 `prune-snapshots` calls between 13:20 and 14:44 (about 1.6M rows and 80 s each). The health check read HEALTHY at 14:47 with the oldest raw row at 30.8 days and the daily forecast ran at 14:53, the first since 08-31. The file stays at 150 GB with 43% of its pages on the freelist
- The last pre-drain hourly run (09-20 12:16) took 63 minutes, 61 of them in the drift check (data drift 38, error drift 23 to produce n=0); the post-drain timing #162 asks for comes from the runs after the daily finished
- `logs/sleep_back.log` holds no SUSPEND line in its history, and from 09-01 to 09-20 it logged "health alert present" every hour of the night window, so the retention alert #161 describes also kept the box awake for twenty nights
...
- The comment in .github/labeler.yml carried the same wrong premise in writing, mapping `^dependabot/` to chore "because dependabot.yml sets `labels: []`". It now says a bump is a chore, and records that dependabot.yml names both labels directly, so the branch rule is a backstop rather than the only path. The header comment in .github/workflows/labeler.yml carried a third copy of it, claiming a bump "arrives with no labels at all", and is corrected in the same pass
- [#143](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/143) moves to M0A's gated tail in the same pass, since shipping the fix is what converts it into an issue waiting on a date. Its check needs a real bot PR, and both ecosystems are monthly, so the earliest is 2026-09-01. Deleting the leftover default labels waits for that same run, because doing it first removes the evidence that the fix worked
...
- The scheduled nudge for `waiting: wall clock` issues leaves the roadmap Backlog and is filed as [#146](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/146). It had been deferred with "two issues carry the label today, which is not enough to justify building it. Revisit if the pattern recurs", and both halves of that condition fired: [#11](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/11)'s stated check date of 2026-08-03 passed with nothing surfacing it, and [#143](https://github.com/RussellFeinstein/WoW-Economy-Forecaster/issues/143) becomes a third carrier once its fix ships
