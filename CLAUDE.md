...
## ACTIVE OPERATIONAL HAZARD (2026-07-12, updated 2026-07-19)
- **Hourly ingestion has been dead since 2026-04-15**: a crashed run leaked `data/db/.hourly.lock`. Until v2.4.9, run_hourly.bat logged SKIPPED and exited 0 on every run with the lock present; it now takes over locks older than 180 minutes (issue #3). Last successful ingest 2026-04-07. The daily forecast task no longer forecasts from the frozen data: as of v2.5.0 (issue #12) run_daily.bat and ForecastStage both refuse when the newest observation is older than 26h, so the task now fails fast until ingestion is restored.
...
- **Do NOT delete the lock file or re-enable the hourly task outside the runbook in issue #1.** Daily history survives only in daily_rollup_archetype/daily_rollup_item, which are incomplete (22 dates, 2026-02-24..2026-04-07; 2026-03-20..2026-04-06 missing). Back up and backfill rollups first. Re-running setup_tasks.bat is safe since v2.7.1: it preserves the task's Disabled state; runbook step 9 must therefore enable it explicitly (`schtasks /Change /TN WoWForecaster-Hourly /ENABLE`).
...
## Test Count
All 1346 tests passing locally as of 2026-07-19. Twenty-four of them (5 run_hourly.bat lock-guard tests from issue #3, 4 run_daily.bat freshness-gate tests from issue #12, 8 run_healthcheck.bat alerting tests from issue #4, and 7 setup_tasks.bat registration tests from issue #6, all in tests/test_scripts/) are Windows-only and skip on the Linux CI runners, so CI reports 1322 passed + 24 skipped, green on Python 3.11 and 3.12. Issues #7, #8, #49 closed the green-CI tier: 8 item-forecast date anchors, the pruner boundary flake plus one new boundary test, the health-report UTC anchor, and 2 scheduler CLI-locator tests now parametrized over both venv layouts.
