...
## Roadmap
Next-phase work (M0 restore/harden ops -> M0.5 unattended capture -> M1 model validation -> M2 PostgreSQL+dbt warehouse -> M3 Power BI/Tableau -> M4 paper-trading P&L + ranking A/B -> M5 event impact study -> M6 publish) is tracked in [docs/ROADMAP.md](docs/ROADMAP.md) and GitHub milestones M0-M6 plus M0.5 (issues #1-#43, created 2026-07-12). Session protocol: pick the lowest open milestone, work its issues in order; M0.5 issues #41-#42 depend on nothing local and may start before the M0 runbook.
