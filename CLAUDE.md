...
- [scripts/setup_tasks.bat](scripts/setup_tasks.bat) — registers four tasks, all silent via wscript.exe + run_silent.vbs (vbs waits and propagates the exit code): WoWForecaster-Hourly (hourly, /ST pinned to :16 — daily-collision avoidance + snapshot-edge sampling; also matches the cloud cron's first firing each hour, though the densified :16/:36/:56 cloud schedule weakens the alignment rationale), WoWForecaster-Daily (07:00), WoWForecaster-HealthCheck (/SC HOURLY /MO 6 /ST 00:45 — 29 min clear of the :16 ingest, done before 07:00), WoWForecaster-Backup (/SC DAILY /ST 07:30 — after the daily forecast so each backup includes that morning's forecasts; issue #80)
...
- Real snapshot 2026-07-23: 118 MB raw / 31 MB gz, 24 tables, ~718K rows, <6s; forecast_outputs (325K) + daily_rollup_item (333K) dominate, so ~31 MB/day and ~11 GB/yr unbounded (exceeds R2 free tier in <1yr). Each backup is a COMPLETE snapshot, so a bounded bucket lifecycle (~90d recommended) loses no rollups; design + restore steps + retention note: [docs/db-backup.md](docs/db-backup.md)
