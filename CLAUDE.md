...
- ForecastStage builds the item-level forecasts on a read-only connection from the rollup tables, then inserts the archetype and item rows in one short write transaction; a write transaction never spans a read of the observation tables (#107). Python's sqlite3 opens the transaction at the first INSERT, so anything read on that connection afterwards is read under the write lock, which is how the daily locked the hourly out for 25 minutes a day. The recommend stage still has that shape (#169)
...
- **Prune stall 2026-08-31 to 2026-09-20 (#155).** Two entries missing from `idx_obs_raw_realm_ingested` made the raw DELETE for the 2026-07-31T06 hour slice fail on every hourly run once it entered the retention window, and the pruner stops at the first failing slice, so about 105M raw rows and their children went past the ToS window, the file grew 98 GB -> 150 GB, and the daily forecast gate (which includes the retention sentinel, #161) skipped every forecast from 09-01. Migration 0012 dropped the index (applied on rex-desktop 2026-09-20, 271 s) and the health probe moved to the normalized table; the first prune afterward cleared the damaged slice in 79 s (evidence on #155), and the 104M-row remainder was cleared the same afternoon by 55 `prune-snapshots` calls between 13:20 and 14:44 (about 1.6M rows and 80 s each); the health check read HEALTHY at 14:47 with the oldest raw row at 30.8 days, and the daily forecast ran at 14:53, the first since 08-31. Two consequences persist: the file stays at 150 GB with 43% of its pages on the freelist for inserts to reuse (15.8M of 36.7M pages; C: had 30 GB free at the time, so no VACUUM INTO is possible and none is wanted on this box), and the hourly run's drift check had grown past the hour on the oversized table, silently halving local capture (#162; cloud capture held the hours). The drain did not shorten it: the first clean run on the 30-day table (2026-09-20 17:16) took 63.7 minutes, 59 of them in the drift check, against 63 on the 45-day table that morning, and the 18:16 trigger was dropped behind it. Whatever the drift check spends its time on, it is not the rows the retention window removes, so #162 moved up to row 7 of the M0A order.
- **Every daily forecast run locks the 07:16 hourly out until #169 lands.** Until v2.14.28 the forecast stage scanned the whole normalized table inside its write transaction (23 to 38 minutes), so the 07:16 hourly's IngestStage failed with `database is locked` on every day the daily ran (08-29, 08-30, 08-31, 09-20) and reported `[PARTIAL]` at exit 0 (#164). v2.14.28 (#107) moved the item-forecast reads onto the rollup tables and ahead of the write transaction, which makes the forecast stage sub-second; the recommend stage that follows it has the same shape (its item-overlay enrichment scans 3-day windows of the normalized table inside the transaction the recommendation inserts open, 43 to 61 minutes per run) and becomes the 07:16 lock holder the moment the forecast stage stops being one (#169, found by the #107 re-audit). Cloud capture holds those hours. #107 stays open with `waiting: wall clock` until the first daily-run day after both fixes shows a clean 07:16 ingest in `logs/hourly.log`. Found by the 2026-09-20 audit, which also found four single-character corrupt rows in the rollup tables that no integrity check can see (#165), and reopened #100, which had been closed unfixed since 07-30.
