...
- All 20 modules are authored (347 questions total): M06 pilot in v2.11.0, the other 19 filled in by a verified agent fleet in v2.11.1, plus m03-q19 added by issue #123 in v2.14.13. A bank grows when the code it teaches changes: #123 broke one M03 anchor and put three more answers slightly out of date, and the response was to re-anchor and extend them rather than delete, because the module outlives the defect it was written against. The curriculum still reports "not authored yet" for any declared module without a bank, which is the state a future milestone re-enters by adding a module before its bank; a bank with no declared module is an integrity failure
